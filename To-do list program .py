...
class TodoAppGUI:
    ...
    def add_task(self):
        ...
        priority_combo = ttk.Combobox(form_frame, textvariable=priority_var, values=["High", "Medium", "Low"], width=10, state="readonly")
        ...
        # Tag selection
        ttk.Label(form_frame, text="Tags:", font=('Arial', 10, 'bold')).grid(row=4, column=0, sticky=tk.W, pady=5)
        
        # Use Checkbuttons to implement multiple selection
        tag_vars = {}
        for i, tag in enumerate(["Work", "Personal", "Study", "Urgent"]):
            tag_vars[tag] = tk.BooleanVar(value=False)
            ttk.Checkbutton(form_frame, text=tag, variable=tag_vars[tag]).grid(
                row=4, column=1 if i < 2 else 2, sticky=tk.W, pady=2, padx=(0 if i < 2 else 10)
            )
    ...
    def edit_task(self):
        ...
        priority_combo = ttk.Combobox(form_frame, textvariable=priority_var, values=["高", "中", "低"], width=10, state="readonly")
        priority_combo.grid(row=2, column=1, sticky=tk.W, pady=5)
    ...
    def view_task(self, event):
        ...
        ttk.Label(details_frame, text="状态:", font=('Arial', 10, 'bold')).grid(row=5, column=0, sticky=tk.W, pady=5)
